...
module.exports = {
  ...
  rules  : {
    ...
    'max-params'             : ['error', { max: 3 }], // 限制函数定义中最大参数个数
    ...
    'newline-per-chained-call': ['warn', { ignoreChainWithDepth: 2 }], // 要求方法链中每个调用都有一个换行符
    ...
    complexity              : ['error', { max: 10 }], // 限制圈复杂度
    ...
    'import/exports-last'               : ['error'], // 要求导入语句之后没有其他语句
    'import/no-duplicates'              : ['warn'], // 禁止同个文件多次导入相同的模块
    'import/no-namespace'               : ['off'], // 强制要求不能使用* as 语句
    'import/extensions'                 : ['error', 'never'], // 要求统一的导入文件后缀模式
    ...
    'import/no-unassigned-import'       : ['error', { allow: [] }], // 要求配置允许导入未分配的模块列表
    ...
    'import/no-relative-packages'      : ['error'], // 禁止通过相对路径导入包
    ...
    'import/no-extraneous-dependencies': ['error'], // 禁止导入package.json中未声明的模块
